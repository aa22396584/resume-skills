...
def _long_description() -> str:
    ...
    return (
        ...
        "agent's own on-disk session store, selects the session that matches "
        "the current working directory, and prints an inert, best-effort "
        "redacted markdown handoff for you to review before pasting it into "
        "the new session. Everything runs locally with Python's standard "
        "library: the bundled reader never contacts the network, never "
        "invokes the source agent's CLI, never modifies the source store, and "
        "ships no MCP server, credentials, scheduler or background service. "
        "Recovered text is marked untrusted and stale, and redaction is "
        "best-effort rather than complete data-loss prevention. This is "
        "offline context migration, not live session restore."
    )
